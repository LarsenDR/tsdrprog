...
const protocol string = "TangerineSDR"
const update string = "2021-7-14"

//  String to define the program banner in each web window
const banner string = `
<div id="header" align="left" style="background-color:Orange;border:2px solid; border-radius:8px; ">
<b align="left" width="90%" style="margin-left:45px; color:black; font-size:38px ">TangerineSDR Programmer</b>
<p style="margin-left:45px; color:black; font-size:12 ">By Dave, KV&#216S  - Version {{.Version}}, Protocol {{.Protocol}} - Last Updated {{.Update}} -  <a style="color:black; font-size:12px" href="http://TangerineSDR.com">TangerineSDR.com</a> </p>
</div>
`
...
<p> The TangerineSDR Programmer is a tool to load {{.Protocol}} protocol firmware into a TangerineSDR radio.  It perform the following tasks:
<ul>
		  <li>Discovery of the TangerineSDR radios available.</li>
		  <li>Erase and Program a selected TangerineSDR radio FPGA.</li>
</ul>
`
...
func introhandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<button class=\"btn\" type=\"submit\" name=\"nic\" value=\"nic\"> Discover</button>")
	fmt.Fprintf(w, "</form>")
	fmt.Fprintf(w, "</td>")
	fmt.Fprintf(w, "<td>")
...
func main() {
	...
	http.Handle("/", http.FileServer(http.Dir("/home/kv0s/drl/src/larsendr/tsdrprog/images/")))
